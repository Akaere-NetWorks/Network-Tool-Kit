use dig_lib::{
    dns::{build_query, QueryConfig, Rdata, RecordType},
    resolver::{send_query, ResolverConfig, ServerAddr},
};

fn google_dns() -> ServerAddr {
    ServerAddr::UdpTcp("8.8.8.8:53".parse().unwrap())
}
...
async fn query_google_com_a() {
    ...
    let result = send_query(&google_dns(), &q, &resolver_cfg(false))
        .await
        .unwrap();
...
async fn query_google_com_aaaa() {
    ...
    let has_aaaa = result
        ...
        .answers
        .iter()
        .any(|rr| matches!(rr.rdata, Rdata::Aaaa(_)));
...
async fn query_google_com_mx() {
    ...
    let has_mx = result
        ...
        .answers
        .iter()
        .any(|rr| matches!(rr.rdata, Rdata::Mx { .. }));
...
async fn query_via_tcp() {
    ...
    let result = send_query(&google_dns(), &q, &resolver_cfg(true))
        .await
        .unwrap();
    assert!(result.message.header.is_response());
}
